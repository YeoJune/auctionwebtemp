  ...
  async cleanupOldValueItems(daysThreshold = 90, batchSize = 500) {
    ...
    try {
      ...
      oldItems.forEach(item => {
        ...
        if (item.additional_images) {
          try {
            JSON.parse(item.additional_images).forEach(img => imagesToDelete.add(img));
          } catch (error) {
            console.error(`Error parsing additional_images for item ${item.item_id}:`, error);
          }
        }
      });
      ...
      console.log(`Cleaned up ${oldItems.length} items and their associated images`);
    } catch (error) {
